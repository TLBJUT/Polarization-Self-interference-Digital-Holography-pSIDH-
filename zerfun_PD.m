...
% 
% ==============================================================================
% # This script provides the Zernike pipil function generation for
...
function [p, pd]= zerfun_PD(PARAMS,c,defo)
% global PARAMS;

phasemap = ones(PARAMS(8),PARAMS(1));
...
for ny = 1:PARAMS(1)
	...
	for nx = 1:PARAMS(8)
		...
		xpos = ((nx-1)*(sizeoffield/PARAMS(8))-(sizeoffield/2));
	...
	if norm_radius > PARAMS(2)/PARAMS(3)
         ...
         SCfactor=B+A*10^(-p*(PARAMS(2)^2)*d^2);
      	...
      	phasemap(nx,ny) = exp(1i * phase);
        phasemap_d(nx,ny) = SCfactor * exp(-1i * 2 * pi * (phase+defo*sqrt(3)*((2)*r^2+(-1)*r^0))/PARAMS(5));
			PARAMS(6) = PARAMS(6) + 1;
		end
	end
end
